...
__global__ void conv_forward_kernel(float *output, const float *input, const float *mask, const int B, const int M, const int C, const int H, const int W, const int K, const int S)
{
    ...
    */
    ...
    // (void)Width; // silence declared but never referenced warning. remove this line when you start working
    ...
    if (h < H_out && w < W_out) {
        for (int c = 0; c < C; c++) {
            for (int p = 0; p < K; p++) {
                for (int q = 0; q < K; q++) {
                    acc += in_4d(b_out, c, p + h*S, q + w*S) * mask_4d(m, c, p, q);
                }
            }
        }
        out_4d(b_out, m, h, w) = acc;
    }
    #undef out_4d
    #undef in_4d
    #undef mask_4d
